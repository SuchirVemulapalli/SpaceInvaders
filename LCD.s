...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	PUSH {R1-R6, LR}
step1
	LDR R5, =SSI0_SR_R ; load address
	LDR R1, [R5] ; load content
	AND R1, #0x10 ; isolate bit 4
	CMP R1, #0x10 ; if bit 4 is 1 keep looping
	BEQ step1
	LDR R3, =GPIO_PORTA_DATA_R ; D/C pa6 address
	LDR R4, [R3] ; load content
	BIC R4, #0x40; clear bit 6
	STR R4, [R3]
	LDR R5, =SSI0_DR_R
	STRB R0, [R5]
step5
	...
	LDR R1, [R6]
	AND R1, #0x10 ; isolate bit 4
	CMP R1, #0x10 ; if bit 4 is 0 keep looping
	BEQ step5
...
backagain
	...
	LDR R3, =GPIO_PORTA_DATA_R ; D/C pa6 address
	LDR R4, [R3] ; load content
	ORR R4, #0x40 ; set bit 6 to one
	STR R4, [R3] ; store into ssio
	LDR R3, =SSI0_DR_R
	STRB R0, [R3]
	POP {R1-R5, LR}
